...
![1552532971644](0.9120775672132797.png)

![1552532971644](0.3597541305746248.png)
...
这种方式可以通过 Vuex 或者 Event Bus 解决，另外如果你不怕麻烦的话，可以使用这种方式解决所有的通信情况
...
常用: 父组件通过 `props` 传递数据给子组件，子组件通过 `emit` 发送事件传递数据给父组件,父组件监听事件。
...
`computed` 是计算属性，依赖其他属性计算值，并且 `computed` 的值有缓存，只有相关依赖发生改变时才会重新取值
